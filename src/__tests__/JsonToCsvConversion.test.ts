...
test('Individual Object WhiteList', () => {
  const jsonToCsvConversion = new JsonCsvConverter().convertJsonToCsv(SimplePersonJson[0], {whiteList: ['firstName', 'lastName']});
  expect(jsonToCsvConversion.values).toStrictEqual([['John', 'Smith']]);
});

test('Individual Object BlackList', () => {
  const jsonToCsvConversion = new JsonCsvConverter().convertJsonToCsv(SimplePersonJson[0], {blackList: ['firstName', 'lastName']});
